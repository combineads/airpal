...
function getRuns(user) {
  if (user) {
    return RunStore.getFetchCollection().where({
      user
    }, {
      sort: true
    })
  } else {
    return RunStore.getFetchCollection().all({
      sort: true
    });
  }
}
...
let FetchRunsTable = React.createClass({
  displayName: 'FetchRunsTable',
